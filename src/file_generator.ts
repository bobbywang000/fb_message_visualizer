...
export class FileGenerator {
  ...
  writeBackupJson(): void {
    write(
      join(this.outputFolder, this.backupJsonPath),
      this.generateBackupJson()
    );
  }
  ...
  writeAllHtmlSegments(): void {
    splitArray(this.messages, this.MAX_MESSAGES_PER_SEGMENT).forEach(
      (segment) => {
        write(
          this.htmlFilename(segment),
          this.generateHtmlSegment(this.generateMessageDivs(segment))
        );
      }
    );
  }
...
${messageDivs.join("\n")}
  ...
  private htmlFilename(segment): string {
    ...
    return join(
      this.outputFolder,
      `${transformTimestampToIsoString(
        first.epochMs
      )}_${transformTimestampToIsoString(last.epochMs)}.html`
    );
  }
