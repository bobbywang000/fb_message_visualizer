...
const formatConversation = (nameUtils: NameUtils, formatter: MessageContentFormatter) => {
    return (currChat) => new Conversation(read(currChat), nameUtils, formatter).getInternalRepresentation();
};

const mergeConversations = (acc, conversation) => {
    return {
        messages: acc.messages.concat(conversation.messages),
        participantsHash: acc.participantsHash.concat(conversation.participantsDigest),
    };
};
